    ...
    try:
        ...
        if post_type == 'text':
            ...
            if content_size > max_size:
                return jsonify({'success': False, 'message': 'Teks twò long (10MB max)'}), 400
        ...
        if post_type == 'url':
            ...
            if not url_pattern.match(content):
                return jsonify({'success': False, 'message': 'URL envalid'}), 400
...
def preview_url():
    ...
    try:
        ...
        if not url:
            return jsonify({'success': False, 'message': 'URL obligatwa'}), 400
        ...
        # Clean up the data
        metadata['title'] = metadata['title'][:100] if metadata['title'] else 'Pa gen tit'
        metadata['description'] = metadata['description'][:200] if metadata['description'] else 'Pa gen deskripsyon'
